...
class OrderReservationConcurrencyTest {
    ...
    @Autowired private lateinit var benchmarkService: OrderBenchmarkService
    ...
    fun `pessimisticLock - 재고보다 많은 동시 요청에서 정확히 재고만큼만 성공`() {
        val (success, fail) = runConcurrent { memberId ->
            benchmarkService.createWithPessimisticLock(memberId, productId, 1)
        }
    ...
    fun `skipLocked - 재고보다 많은 동시 요청에서 정확히 재고만큼만 성공`() {
        ...
        val (success, fail) = runConcurrent { memberId ->
            benchmarkService.createWithSkipLocked(memberId, productId, 1)
        }
    ...
    fun `confirmed reservation is not expired and its stock stays claimed`() {
        saveUnitStocks(1)
        val reservation = benchmarkService.createWithSkipLocked(1L, productId, 1)
    ...
    fun `order confirmation and cancellation retries are idempotent`() {
        saveUnitStocks(1)
        val reservation = benchmarkService.createWithSkipLocked(2L, productId, 1)
    ...
    fun `cancellation releases only stock owned by that reservation`() {
        ...
        val reservation = benchmarkService.createWithSkipLocked(memberId, productId, 2)
    ...
    fun `bulk stock release rolls back when reservation quantity does not match`() {
        saveUnitStocks(1)
        val reservation = benchmarkService.createWithSkipLocked(8L, productId, 1)
    ...
    fun `confirm and cancel race has exactly one consistent winner`() {
        saveUnitStocks(1)
        val reservation = benchmarkService.createWithSkipLocked(11L, productId, 1)
    ...
    fun `expired pending reservation restores its exact stock`() {
        saveUnitStocks(2)
        val reservation = benchmarkService.createWithSkipLocked(21L, productId, 2)
    ...
    fun `product query reports unit stock availability instead of benchmark stock column`() {
        ...
        val reservation = benchmarkService.createWithSkipLocked(22L, productId, 1)
    ...
    fun `database constraint allows only one pending reservation per member and product`() {
        ...
        val threads = (1..30).map {
            Thread {
                try {
                    start.await()
                    benchmarkService.createWithSkipLocked(31L, productId, 1)
