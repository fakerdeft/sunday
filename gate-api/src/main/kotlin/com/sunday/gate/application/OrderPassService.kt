 ...
 * 주문 서버 앞단 게이트.
 *
 * budget 은 스케줄러가 주기마다 재계산하고, 그 사이에는 차감만 한다.
 * 요청 처리 중 재고를 다시 읽으면 낡은 재고와 최신 발급 수가 섞여 재고보다 많이 통과한다.
 *
 * budget 은 DB 재고의 파생값. 최종 판정은 주문 서버의 `SKIP LOCKED`.
 */
@Service
class OrderPassService(
    ...
    companion object {
        ...
        /**
         * 만료분 회수 후 같은 시점의 값으로 budget 을 한 번에 정한다.
         *
         * KEYS: budget, inFlight
